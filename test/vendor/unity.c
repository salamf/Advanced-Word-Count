...
const char UnityStrActual[]                 = " Actual   ";
...
//Amanda
static void UnityPrintExpectedAndActualMultiLineStrings(const char* expected, const char* actual);
static void UnityPrintExpectedAndActualMultiLineStrings(const char* expected, const char* actual)
{
    UNITY_PRINT_EOL;
    UnityPrint(UnityStrExpected);
...
//-----------------------------------------------
//Amanda
void UnityAssertEqualLongString(const char* expected,
                            const char* actual,
                            const char* msg,
                            const UNITY_LINE_TYPE lineNumber)
{
    ...
    char failed_expected[truncated+5] = {0};
    char failed_actual[truncated+5] = {0};
    ...
    if (expected && actual)
    {
        for (i = 0; expected[i] || actual[i]; i++)
        {
            ...
            {
                Unity.CurrentTestFailed = 1;
                int idx = i > truncated ? i-truncated : 0;
                int len = i > truncated ? truncated+1 : i+1;
                //printf("i=%d, idx=%d, len=%d\n", i, idx, len);  
                strncpy(failed_expected, &expected[idx], len);
                //strncat(failed_expected, indicator, strlen(indicator));
                strncpy(failed_actual, &actual[idx], len);
                //strncat(failed_actual, indicator, strlen(indicator));
                break;
            }
        }
    }
    ...
    if (Unity.CurrentTestFailed)
    {
      UnityTestResultsFailBegin(lineNumber);
      //UnityPrintExpectedAndActualStrings(expected, actual);
      UnityPrintExpectedAndActualMultiLineStrings(failed_expected, failed_actual);
      UnityAddMsgIfSpecified(msg);
      UNITY_FAIL_AND_BAIL;
    }
}
